...
meta_df = meta_df.set_index("donor_id")
...
for folder in sample_folders:
    ...
    group_values = meta_df.loc[sample_id, 'donor_id'] if sample_id in meta_df.index else 'Unknown'
    ad.obs['donor_id'] = [group_values] * ad.n_obs
...
print(all_obs['donor_id'].value_counts())

meta_df['donor_id'].value_counts()
...
for ad in adatas:
    sample = ad.obs['GSM_ID'].iloc[0]
    group = ad.obs['donor_id'].iloc[0]
    n_cells = ad.n_obs
    print(f"Sample: {sample} | donor_id: {group} | Cells: {n_cells}")
...
# ========== Load GSM ID mapping ==========
# Ensure this file has exact rows in order: GSM_ID, donor_id
meta_df = pd.read_csv("sample_to_donor.csv")  # adjust path if needed
...
metadata_csv = "sample_to_donor.csv"  # Must have columns: GSM_ID, Sample_ID
os.makedirs(out_dir, exist_ok=True)

# ========= Load Metadata =========
meta = pd.read_csv(metadata_csv)
sample_to_gsm = dict(zip(meta["GSM_ID"], meta["donor_id"]))
...
meta_df = pd.read_csv("sample_to_donor.csv")  # Ensure this has GSM_ID and Group
...
sc.pl.umap(adata_all, color="donor_id", save="_by_donor_id.png", title="UMAP by donor_id")
...
# Load metadata (must match order of loading samples!)
meta_df = pd.read_csv("sample_metadata_4.csv")  # Ensure this has GSM_ID and Group
